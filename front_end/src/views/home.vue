...
const sortFileList = () => {
  ...
  if (value2.value) {
    if (value3.value === "文件名")
      sortedList = sortedList.filter((file) =>
        file.file.name.includes(value2.value)
      );
    else if (value3.value === "地点")
      sortedList = sortedList.filter((file) =>
        file.place.includes(value2.value)
      );
    else if (value3.value === "设备")
      sortedList = sortedList.filter((file) =>
        file.device.includes(value2.value)
      );
    else
      sortedList = sortedList.filter((file) =>
        file.file.sort.some((tag) => tag.includes(value2.value))
      );
  }
...
const handleAfterRead = async (files) => {
  ...
  for (const file of files) {
    const index = fileList.value.findIndex(
      (item) => item.objectUrl === file.objectUrl
    );
    if (index !== -1) {
  ...
  if (files.length === 1) {
    ...
    axios
      .post("http://localhost:8080/images/addBatch", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
...
const handleAfterDelete = (file) => {
  console.log(file);
  const index = fileList.value.findIndex(
    (item) => item.uploadTime === file.uploadTime
  );
  if (index !== -1) {
...
const readFile = (file) => {
  ...
  if (index !== -1) {
    ...
    pictureName.value = selectedFile.file.name
      ? selectedFile.file.name
      : "未知";
    pictureSize.value = selectedFile.file.size
      ? toSize(selectedFile.file.size)
      : "未知";
    pictureTime.value = selectedFile.file.lastModified
      ? formatTimestamp(selectedFile.file.lastModified)
      : "未知";
    ...
    picture.value = selectedFile.objectUrl
...
const readFile2 = (item2) => {
  ...
  pictureTime.value = item2.lastModified
    ? formatTimestamp(item2.lastModified)
    : "未知";
...
const fetchFolderData = () => {
  axios
    .get("http://localhost:8080/images/folderData")
    .then(async (response) => {
              ...
              if (extension === "dng" || extension === "raw") {
                try {
                  ...
                  const rawResponse = await axios.post(
                    "http://localhost:8080/images/processRaw",
                    formData
                  );
...
const readFile3 = (file) => {
  try {
    ...
    pictureTime.value = file.file?.lastModified
      ? formatTimestamp(file.file.lastModified)
      : "未知";
...
//图片详情
const picture = ref(defaultPic);
const pictureName = ref("图片名");
const pictureSize = ref("未知");
const pictureTime = ref("未知");
const pictureDevice = ref("未知");
const picturePlace = ref("未知");
const pictureSort = ref(["标签"]);
</script>
...
<template>
    ...
    <div class="tab">
      <van-tabs v-model:active="active" shrink type="card" line-width="100vw">
        ...
        <van-tab title="照片导入" class="input">
          ...
          >
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                >
                <el-dropdown-item class="listButton" @click="sortList2('地点')"
                  >地点</el-dropdown-item
                >
                <el-dropdown-item class="listButton" @click="sortList2('设备')"
                  >设备</el-dropdown-item
                >
                <el-dropdown-item class="listButton" @click="sortList2('标签')"
                  >标签</el-dropdown-item
                >
              </el-dropdown-menu>
            </template>
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item
                  class="listButton"
                  @click="sortList('导入顺序')"
                  >导入顺序</el-dropdown-item
                >
                <el-dropdown-item
                  class="listButton"
                  @click="sortList('时间顺序')"
                  >时间顺序</el-dropdown-item
                >
                <el-dropdown-item
                  class="listButton"
                  @click="sortList('大小顺序')"
                  >大小顺序</el-dropdown-item
                >
                <el-dropdown-item
                  class="listButton"
                  @click="sortList('文件名顺序')"
                  >文件名顺序</el-dropdown-item
                >
                <el-dropdown-item
                  class="listButton"
                  @click="sortList('其他顺序')"
                  disabled
                  >其他顺序</el-dropdown-item
                >
              </el-dropdown-menu>
            </template>
          </el-dropdown>
          <div class="box"></div>
          <van-uploader
            v-model="filteredFileList"
            preview-size="10.5vw"
            :preview-full-image="false"
            class="up"
            @click-preview="readFile"
          ...
          <van-uploader
            ...
            multiple
            @click-preview="readFile"
            accept="image/*,.cr2,.cr3,.nef,.nrw,.arw,.srf,.sr2,.raf,.orf,.rw2,.pef,.dng,.raw,.srw,.kdc,.erf,.rwl,.mef,.tif,.tiff"
            :preview-image="false"
          >
            <template #preview-cover="{ file }">
              <div class="preview-cover van-ellipsis">
        ...
        <van-tab title="分类预览" class="preview">
          ...
          >
            ...
            <div
              class="pictureList"
              :ref="(el) => (scrollContainers[index] = el)"
              @scroll="() => handleScroll(index)"
            >
              <van-image
                v-for="item2 in item.pictureList"
                :key="item.listName"
                class="picture"
                width="150px"
                :src="item2.objectUrl"
                ...
                "
                @click="readFile2(item2)"
              />
            </div>
            <button
              class="arrowButton"
              v-show="!isAtEnd[index]"
        ...
        <van-tab v-for="(item, index) in moreList" class="input">
          <template #title
            >{{ item.listName
            }}<button class="closeButton" @click="deleteTabs(index)">
              <van-icon name="close" /></button
          ></template>
          <div class="box"></div>
          <van-uploader
            ...
            preview-size="10.5vw"
            :preview-full-image="false"
            @click-preview="readFile3"
            @delete="handleAfterDelete"
          >
            <template #preview-cover="{ file }">
              <div class="preview-cover2 van-ellipsis">
                {{ file.name || "未命名图片" }}
              </div>
            </template>
            <div></div>
  ...
  <div class="nav">
    ...
    <div class="littleSize" v-show="picturePlace">
      拍摄地点：{{ picturePlace }}
    </div>
    <div class="littleSize" v-show="pictureDevice">
      拍摄设备：{{ pictureDevice }}
    </div>
    <div class="littleSize">拍摄时间：{{ pictureTime }}</div>
    <div class="classficiation">
      <div class="tag" v-for="(item, index) in pictureSort" :key="index">
        <van-tag
          round
          size="large"
